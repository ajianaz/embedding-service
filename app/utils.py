...
def save_to_qdrant(embedding, text, collection_name, payload={}):
    if not QDRANT_ENABLE:
        return
    ...
    logger.info(f"✅ Saved data to Qdrant with ID {point_id}")

# Fungsi untuk mencari di Qdrant
def search_in_qdrant(embedding, collection_name, top_k=3):
    if not QDRANT_ENABLE:
        return []
